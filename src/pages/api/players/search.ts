  ...
  try {
    ...
    playersSnap.forEach(doc => {
      ...
      results.push({
        ...
        isRostered: rosterTeamName !== undefined,
        rosterTeamName,
      });
    });

    // Sort alphabetically by name; cap the result set since this is a search endpoint, not a listing.
    results.sort((a, b) => a.fullName.localeCompare(b.fullName));
    const limited = query ? results.slice(0, MAX_RESULTS) : results.slice(0, MAX_RESULTS);
